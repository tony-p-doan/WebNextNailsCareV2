...
import {addReward} from "../../../data/rewardsApi";
...
export function AddRewardPage() {
  ...
  useEffect(() => {
    ...
    listServices(storeId).then((list) =>
      setServices(list.filter((s) => serviceIsComplete(s)))
    );
  }, [storeId]);
  ...
  return (
    <>
      <AppBar title="Add Reward" backTo="/store-admin/rewards" />
      <div className="page-bg">
        <main className="container" style={{paddingTop: "1.5rem"}}>
          <div className="form-card">
            <div className="form-group">
              <label>Reward Name</label>
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Free Classic Manicure" />
            </div>
            <div className="form-group">
              <label>Description</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the reward…" />
            </div>
            <div className="form-group">
